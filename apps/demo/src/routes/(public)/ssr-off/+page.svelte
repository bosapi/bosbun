...
<div class="space-y-6 max-w-2xl">
    ...
    <p class="text-muted-foreground">
        This page reads <code>window</code> at module top-level. If SSR ran on the
        server it would crash with <code>ReferenceError: window is not defined</code>.
        It works because <code>+page.server.ts</code> exports <code>ssr = false</code>.
    </p>
    ...
    <section class="space-y-2 border-t pt-4 text-sm text-muted-foreground">
        ...
        <ol class="list-decimal pl-5 space-y-1">
            <li>View source — <code>&lt;div id="app"&gt;&lt;/div&gt;</code> is empty (no SSR body).</li>
            <li>View source — script contains <code>window.__BOSIA_SSR__=false</code> and the <code>load()</code> data.</li>
            <li>Page renders after hydration with no SSR mismatch warnings in the console.</li>
            <li>Other pages (e.g. <code>/</code>) still SSR normally — view source shows rendered markup.</li>
        </ol>
    </section>
</div>
